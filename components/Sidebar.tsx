...
const Sidebar: React.FC<SidebarProps> = ({ isOpen, onClose }) => {
  ...
  const commonLinkClass = "flex items-center px-4 py-3 text-sm font-medium text-gray-400 rounded-xl transition-all duration-200 group hover:bg-slate-800 hover:text-indigo-400";
  const activeLinkClass = "bg-indigo-600/10 text-indigo-500 shadow-none border border-indigo-600/20";
  ...
  return (
    <>
      ...
      <aside className={`fixed top-0 left-0 h-full w-72 bg-slate-950 border-r border-slate-800 flex flex-col z-50 transform transition-transform duration-300 ease-in-out md:translate-x-0 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        {/* Header */}
        <div className="flex justify-between items-center h-16 px-6 border-b border-slate-800/50">
          <div className="flex items-center gap-2">
             <div className="w-8 h-8 bg-indigo-600 rounded-lg flex items-center justify-center text-white font-bold shadow-lg shadow-indigo-900/50">TM</div>
             <span className="text-lg font-bold text-white tracking-tight">TaskMint</span>
          </div>
        ...
        <nav className="flex-1 flex flex-col gap-1 p-4 overflow-y-auto no-scrollbar">
          ...
          <div className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2 px-4 mt-2">Daily Operations</div>
          ...
          <NavLink to="/withdrawals" onClick={handleLinkClick} className={({ isActive }) => `${commonLinkClass} ${isActive ? activeLinkClass : ''}`}>
            <ArrowUpFromLine size={18} className="mr-3" /> Withdrawals
          </NavLink>
          ...
          {/* Tools & System Section (Less Used) */}
          <div className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2 px-4 mt-6">Tools & System</div>
          ...
          <NavLink to="/earning-history" onClick={handleLinkClick} className={({ isActive }) => `${commonLinkClass} ${isActive ? activeLinkClass : ''}`}>
            <History size={18} className="mr-3" /> Earning History
          </NavLink>
